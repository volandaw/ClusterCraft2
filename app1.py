...
# ---------------------------------------------
# STRONA 3 – RAPORT KLASTRÓW (ZBIORCZY)
# ---------------------------------------------
    ...
    if "clustered" not in st.session_state:
        ...
        for cluster_id in sorted(df["Cluster"].unique()):
            cluster_name = cluster_names_list[int(cluster_id)]

            st.subheader(f"Klastr {cluster_id} – {cluster_name}")
            st.write(cluster_info[cluster_name])
            ...
            st.dataframe(cluster_df[[f"pytanie_{i}" for i in range(1, 5+1)]].mean().to_frame("średnia").T)
...
# ---------------------------------------------
    ...
    if "clustered" not in st.session_state:
        ...
        if len(person) == 0:
            ...
            cluster_name = cluster_names_list[cluster_id]

            st.subheader(f"Klient {selected_id}")
            st.write(f"Miasto: **{person['miasto']}**")
            st.write(f"Należy do klastra: **{cluster_name}**")

            st.write("### Średni profil odpowiedzi")
            st.dataframe(
                pd.DataFrame(person[[f"pytanie_{i}" for i in range(1, 6)]].to_dict(), index=["odpowiedź"])
            )

            st.write("### Opis klastra")
            st.info(cluster_info[cluster_name])
...
for cluster_id in sorted(df["Cluster"].unique()):
        ...
        for i in range(1, 6):
                row[f"Śr. pytanie {i}"] = round(cluster_df[f"pytanie_{i}"].mean(), 2)
